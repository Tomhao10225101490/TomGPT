...
def _is_stale_session_error(exc: Exception) -> bool:
    """True when the provider rejected a reused chat/session id (common after abort)."""
    text = str(exc).lower()
    return any(
        key in text
        for key in (
...
def _drop_provider_conversation(
    conversation: JsonConversation | None, provider_name: str
) -> None:
    if conversation is None:
        return
    if hasattr(conversation, provider_name):
        try:
            delattr(conversation, provider_name)
        except Exception:
            pass
        ...
        for _ in range(len(self.providers)):
            ...
            try:
                ...
                if started:
                    provider.live += 1
                    return  # Success
                # Empty stream with no exception — count as failure and rotate
                provider.live -= 1
                empty_err = RuntimeError(
                    f"{provider.__name__} returned empty response"
                )
                exceptions[provider.__name__] = empty_err
                debug.error(f"{provider.__name__} failed: empty response")
                ...
                if _is_stale_session_error(e):
                    debug.log(
                        f"{provider.__name__}: stale session, retrying without conversation"
                    )
                    try:
                        fresh_body = _prepare_provider_kwargs(
                            provider, api_key, None, kwargs
                        )
                        fresh_body.pop("conversation", None)
                        method = get_async_provider_method(provider)
                        response = method(
                            model=alias, messages=messages, **fresh_body
                        )
                        async for chunk in response:
                            if isinstance(chunk, JsonConversation):
                                if conversation is None:
                                    conversation = JsonConversation()
                                setattr(
                                    conversation, provider.__name__, chunk.get_dict()
                                )
                                yield conversation
                            elif chunk:
                                yield chunk
                        ...
                        if started:
                            provider.live += 1
                            exceptions.pop(provider.__name__, None)
                            return
                        provider.live -= 1
                        exceptions[provider.__name__] = RuntimeError(
                            f"{provider.__name__} returned empty response"
                        )
                    except Exception as retry_error:
                        provider.live -= 1
                        exceptions[provider.__name__] = retry_error
                        debug.error(
                            f"{provider.__name__} fresh-session retry failed: {retry_error}"
                        )
                        _drop_provider_conversation(conversation, provider.__name__)
        ...
        **kwargs,
        ...
        for provider in self.get_providers(ignored):
            ...
            try:
                ...
                if conversation is not None and hasattr(conversation, provider.__name__):
                    try:
                        delattr(conversation, provider.__name__)
                    except Exception:
                        pass
                if started:
                    raise e
