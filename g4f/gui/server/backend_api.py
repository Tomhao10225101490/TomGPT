...
from ...tools.export_docx import build_docx_bytes, safe_docx_filename, has_docx
...
class Backend_Api(Api):
    ...
    def __init__(self, app: Flask) -> None:
        ...
        @app.route("/backend-api/v2/export/docx", methods=["POST"])
        def export_docx():
            if not has_docx:
                return (
                    jsonify(
                        {
                            "error": {
                                "message": 'Install "python-docx" | pip install python-docx'
                            }
                        }
                    ),
                    500,
            ...
            title = data.get("title") or "TomGPT"
            filename = safe_docx_filename(data.get("filename"), f"{title}.docx")
            try:
                payload = build_docx_bytes(str(content), title=str(title))
            except Exception as e:
                logger.exception(e)
                return jsonify({"error": {"message": str(e)}}), 500
            headers = {
