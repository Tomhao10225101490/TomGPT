...
def apply_tomgpt_branding(html: str) -> str:
    """Rewrite upstream G4F labels to TomGPT when serving HTML pages."""
    if not isinstance(html, str) or "<" not in html:
        return html
    replacements = (
        ("G4F Chat", "TomGPT"),
        ("GPT4Free", "TomGPT"),
        ("G4F - Framework", "TomGPT"),
        ("G4F - Chat", "TomGPT"),
        ("Welcome to the G4F Web UI!", "欢迎使用 TomGPT！"),
    )
...
def render(filename="home", download_url: str = GITHUB_URL):
        ...
        if os.path.exists(path):
            if path.endswith(".html"):
                ...
                with open(path, "r", encoding="utf-8") as f:
                    html = f.read()
                return apply_tomgpt_branding(
                    html.replace("{{ v }}", str(latest_version))
                )
    ...
    if not os.path.exists(cache_file):
        ...
        if html is None:
            ...
            html = apply_tomgpt_branding(html.replace("{{ v }}", latest_version))
            if is_temp:
                return html
