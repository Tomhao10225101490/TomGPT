...
from ...providers.tomgpt_failover import wrap_with_auto_failover
from ...tomgpt_identity import inject_tomgpt_identity, canned_identity_reply
...
class Api:
        ...
        return [
            {
                ...
                "label": (
                    "TomGPT 自动"
                    if provider.__name__ == "AnyProvider"
                    else getattr(provider, "label", provider.__name__)
                ),
    ...
    def _prepare_conversation_kwargs(self, json_data: dict):
        ...
        # TomGPT: force identity policy on every chat request
        messages = inject_tomgpt_identity(messages)
    ...
    def _create_response_stream(
        ...
        try:
            ...
            # TomGPT: always wrap single providers with automatic failover
            provider_handler = wrap_with_auto_failover(provider or AnyProvider)
            provider = (
                getattr(provider_handler, "__name__", None)
                or getattr(getattr(provider_handler, "preferred", None), "__name__", None)
                or provider
            )
            if "user" in kwargs:
        ...
        # TomGPT hard gate: identity questions never reach underlying models
        canned = canned_identity_reply(kwargs.get("messages"))
        if canned:
            yield self.handle_provider(None, model)
            yield self._format_json("content", canned)
        ...
        try:
            ...
            for chunk in result:
                if isinstance(chunk, ProviderInfo):
                    ...
                    # TomGPT: do not render external login links that navigate away
                    yield self._format_json(
                        "message",
                        f"TomGPT：{getattr(chunk, 'label', '该接口')} 需要登录/API Key，已跳过并继续尝试其他接口…",
                    )
    ...
    def handle_provider(self, provider_handler, model):
        # TomGPT: never expose underlying provider/model names to the UI
...
def get_error_message(exception: Exception) -> str:
    ...
    if "图片生成失败" in text:
        return "图片生成失败，请再试一次。"
    lowered = text.lower()
    # Hide provider dumps / payment / auth internals from end users
    if any(
        key in lowered
        for key in (
            "retryprovider",
