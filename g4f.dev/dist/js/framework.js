...
const renderMarkdown = (content) => {
    ...
    if (Array.isArray(content)) {
        content = content.map((item) => {
            ...
            const isAudio = /\.(wav|mp3)$/i.test(mediaName);
            const isVideo = /\.(mp4|webm)$/i.test(mediaName);
            const isImage = item.width && item.height || /\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(mediaName);
            // Document / file attachment chip (ChatGPT-style), never dump raw UUID buckets
            if (item.bucket_id && !isAudio && !isVideo && !isImage) {
                const size = item.size || parseInt(appStorage.getItem(`bucket:${item.bucket_id}`), 10);
                const label = mediaName || ((navigator.language || "").startsWith("zh") ? "附件" : "Attachment");
                const href = item.url || `#`;
                return `📎 **[${framework.escape(label)}](${href})**${size ? ` (${formatFileSize(size)})` : ""}`;
            }
            if (!item.name) {
                ...
                if (item.bucket_id) {
                    const size = parseInt(appStorage.getItem(`bucket:${item.bucket_id}`), 10);
                    const label = (navigator.language || "").startsWith("zh") ? "附件" : "Attachment";
                    return `📎 **[${label}](${item.url || "#"})**${size ? ` (${formatFileSize(size)})` : ""}`;
                }
            ...
            // Named non-media file (e.g. report.pdf) — show chip, not broken image markdown
            const size = item.size || (item.bucket_id ? parseInt(appStorage.getItem(`bucket:${item.bucket_id}`), 10) : 0);
            return `📎 **${framework.escape(item.name)}**${size ? ` (${formatFileSize(size)})` : ""}`;
        }).join("\n");
    }
